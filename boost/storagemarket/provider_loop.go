		...
		case dealReq := <-p.acceptDealChan:
			...
			sendErrorResp := func(aerr *acceptError) {
				...
				if aerr.isSevereError {
					...
					resp := acceptDealResp{ri: &ProviderDealRejectionInfo{Accepted: false, Reason: aerr.reason}}
				...
				dealReq.rsp <- acceptDealResp{ri: &ProviderDealRejectionInfo{Accepted: false, Reason: aerr.reason}, err: nil}
			}
			...
			if deal.IsOffline && !dealReq.isImport {
				...
				dealReq.rsp <- acceptDealResp{ri: &ProviderDealRejectionInfo{Accepted: true}}
			...
			dealReq.rsp <- acceptDealResp{&ProviderDealRejectionInfo{Accepted: true}, nil}
